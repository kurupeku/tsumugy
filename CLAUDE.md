...
ゲーム画面からトップページ（`/`＝ゲーム選択画面）へ戻る導線を統一する。

#### 設計方針

- **控えめだが認識できる配置**にする（主要コンテンツの邪魔をしない）
- 「始める前に辞めるボタン」のような印象を避ける
- ボタンではなくテキストリンクスタイルを使用（60%不透明度、ホバーで100%）
- 文言は「ゲーム選択に戻る」で統一（ポジティブな表現）

#### 配置ルール

| 画面種別 | 配置位置 | スタイル |
|---------|---------|---------|
| セッション作成画面 | カード外上部（左寄せ） | テキストリンク + 左矢印アイコン |
| 結果画面 | フッター | `btn btn-outline` |
| ファシリテーターダッシュボード | 「セッション終了」で代替 | - |
...
<%# カード外上部に配置 %>
<div class="mb-3">
  <%= link_to root_path, class: "inline-flex items-center gap-1 text-sm text-base-content/60 hover:text-base-content transition-colors" do %>
...
1. セッション作成画面にゲーム選択へ戻るリンクを配置（カード外上部）
2. 結果画面にゲーム選択へ戻るリンクを配置
3. リンク先は常に `root_path`（ゲーム固有のランディングではなくアプリのトップ）
