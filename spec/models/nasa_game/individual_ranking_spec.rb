...
RSpec.describe NasaGame::IndividualRanking, type: :model do
  describe "validations" do
    subject(:ranking) { build(:nasa_game_individual_ranking) }
    ...
    it { is_expected.to validate_uniqueness_of(:item_id).scoped_to(:participant_id) }
  end

  describe "associations" do
    it { is_expected.to belong_to(:participant).class_name("NasaGame::Participant") }
  end

  describe "UUID primary key" do
    it "uses UUID as primary key" do
      ranking = create(:nasa_game_individual_ranking)
  ...
  describe "#item" do
    it "returns the corresponding NasaGame::Item" do
      ranking = create(:nasa_game_individual_ranking, item_id: 1)
  ...
  describe "#error_score" do
    it "calculates the absolute difference between rank and correct rank" do
      ranking = build(:nasa_game_individual_ranking, item_id: 1, rank: 5)
      # Item 1 has correct_rank of 1
      expect(ranking.error_score).to eq(4)
    end

    it "returns 0 when rank matches correct rank" do
      ranking = build(:nasa_game_individual_ranking, item_id: 1, rank: 1)
